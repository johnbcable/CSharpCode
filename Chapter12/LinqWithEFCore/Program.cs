        ...
        static void FilterAndSort()
        {
            using (var db = new Northwind())
            {
                var query = db.Products
                    .ProcessSequence()
                    .AsEnumerable().Where(product => product.UnitPrice < 10M)
                    // IQueryable<product>
                    .AsEnumerable().OrderByDescending(product => product.UnitPrice)
        ...
        static void AggregateProducts()
        {
            using (var db = new Northwind())
            {
               ...
               WriteLine("{0, -25} {1,10:N0}",
                ...
                arg1: db.Products.AsEnumerable().Sum(p => p.UnitsInStock));

               WriteLine("{0, -25} {1,10:N0}",
                arg0: "Sum of units on order:",
                arg1: db.Products.AsEnumerable().Sum(p => p.UnitsOnOrder));
